/**
 * Admin Just Arrived Page
 * Manage all just arrived cars
 */

import { createClient } from '@/lib/supabase/server'
import Link from 'next/link'
import { Plus, Edit, Trash2, Eye, Clock, AlertCircle } from 'lucide-react'
import Button from '@/components/ui/Button'
import Badge from '@/components/ui/Badge'

export const dynamic = 'force-dynamic'

async function getJustArrivedCars() {
  const supabase = await createClient()

  const { data: cars, error } = await supabase
    .from('cars')
    .select(`
      *,
      dealers (name, is_verified),
      car_images (image_url, is_primary)
    `)
    .eq('is_just_arrived', true)
    .order('just_arrived_date', { ascending: false })

  if (error) {
    console.error('Error fetching just arrived cars:', error)
    return []
  }

  return cars || []
}

function getDaysRemaining(justArrivedDate) {
  if (!justArrivedDate) return null

  const date = new Date(justArrivedDate)
  const expiryDate = new Date(date.getTime() + (30 * 24 * 60 * 60 * 1000)) // 30 days
  const today = new Date()
  const daysRemaining = Math.ceil((expiryDate - today) / (24 * 60 * 60 * 1000))

  return daysRemaining
}

export default async function JustArrivedPage() {
  const cars = await getJustArrivedCars()

  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Just Arrived Cars</h1>
          <p className="text-gray-600 mt-2">
            Manage newly arrived cars ({cars.length} active)
          </p>
        </div>
        <Link href="/admin/cars/new">
          <Button variant="primary" className="flex items-center gap-2">
            <Plus size={20} />
            Add Just Arrived Car
          </Button>
        </Link>
      </div>

      {/* Info Banner */}
      <div className="bg-green-50 border-l-4 border-green-500 p-4 mb-6 rounded">
        <div className="flex">
          <Clock className="text-green-500 mr-3" size={24} />
          <div>
            <h3 className="text-sm font-medium text-green-900">Just Arrived Section</h3>
            <p className="text-sm text-green-700 mt-1">
              Cars in this section are automatically removed after 30 days. Use this for newly arrived inventory
              to create urgency and highlight fresh stock.
            </p>
          </div>
        </div>
      </div>

      {/* Cars Table */}
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Car
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Dealer
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Price
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Added Date
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Days Remaining
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Status
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              {cars.length === 0 ? (
                <tr>
                  <td colSpan="7" className="px-6 py-12 text-center">
                    <Clock className="mx-auto text-gray-400 mb-4" size={48} />
                    <p className="text-gray-500 mb-4">No just arrived cars</p>
                    <Link href="/admin/cars/new">
                      <Button variant="primary">Add First Just Arrived Car</Button>
                    </Link>
                  </td>
                </tr>
              ) : (
                cars.map((car) => {
                  const primaryImage = car.car_images?.find(img => img.is_primary) || car.car_images?.[0]
                  const daysRemaining = getDaysRemaining(car.just_arrived_date)
                  const isExpiringSoon = daysRemaining && daysRemaining <= 7

                  return (
                    <tr key={car.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4">
                        <div className="flex items-center">
                          {primaryImage && (
                            <img
                              src={primaryImage.image_url}
                              alt={`${car.make} ${car.model}`}
                              className="w-16 h-16 rounded object-cover mr-4"
                            />
                          )}
                          <div>
                            <div className="text-sm font-medium text-gray-900">
                              {car.year} {car.make} {car.model}
                            </div>
                            <div className="text-sm text-gray-500">
                              {car.condition} • {car.transmission}
                            </div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{car.dealers?.name || 'N/A'}</div>
                        {car.dealers?.is_verified && (
                          <Badge variant="success" size="sm" className="mt-1">Verified Dealer</Badge>
                        )}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                        ₦{parseInt(car.price).toLocaleString('en-NG')}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {car.just_arrived_date
                          ? new Date(car.just_arrived_date).toLocaleDateString('en-NG', {
                              year: 'numeric',
                              month: 'short',
                              day: 'numeric'
                            })
                          : 'N/A'
                        }
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {daysRemaining !== null ? (
                          <div className="flex items-center gap-2">
                            {isExpiringSoon && (
                              <AlertCircle size={16} className="text-orange-500" />
                            )}
                            <span className={`text-sm font-medium ${
                              isExpiringSoon ? 'text-orange-600' : 'text-gray-900'
                            }`}>
                              {daysRemaining} days
                            </span>
                          </div>
                        ) : (
                          <span className="text-sm text-gray-400">N/A</span>
                        )}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex gap-2">
                          <Badge variant="success" size="sm">Active</Badge>
                          {car.is_premium_verified && (
                            <Badge variant="primary" size="sm">Premium</Badge>
                          )}
                          {car.is_verified && (
                            <Badge variant="info" size="sm">Verified</Badge>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <div className="flex items-center gap-3">
                          <Link
                            href={`/cars/${car.id}`}
                            target="_blank"
                            className="text-gray-600 hover:text-gray-900"
                            title="View"
                          >
                            <Eye size={18} />
                          </Link>
                          <Link
                            href={`/admin/cars/${car.id}/edit`}
                            className="text-blue-600 hover:text-blue-900"
                            title="Edit"
                          >
                            <Edit size={18} />
                          </Link>
                          <button
                            className="text-red-600 hover:text-red-900"
                            title="Delete"
                          >
                            <Trash2 size={18} />
                          </button>
                        </div>
                      </td>
                    </tr>
                  )
                })
              )}
            </tbody>
          </table>
        </div>
      </div>
    </div>
  )
}
